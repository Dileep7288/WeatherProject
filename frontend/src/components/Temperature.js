...
const Temperature = () => {
  ...
  const weatherData = async (temp) => {
    try {
      const data = await fetch(
        `${Weather_URL}weather_data/temperature/${temp}`
      );

      const json = await data.json();
      if (json.error) {
        setMessage(`Data Not Found for the Entered Temperature ${temp}`);
        setData([]);
      } else {
  ...
  return (
    <div className="flex flex-col items-center mx-auto my-8 p-4 bg-gray-100 rounded-lg shadow-md max-w-lg">
      <h2 className="text-center text-2xl font-bold mb-6">
        Get Weather Data Using Temperature
      </h2>
      <div className="flex gap-4 justify-center mb-4 ">
        <input
          ...
          ref={inputRef}
          type="text"
          placeholder="Enter City Temperature"
        />
        <img
